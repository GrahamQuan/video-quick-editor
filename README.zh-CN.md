...
在“设置 → 应用更新”点击“检查更新”，查询本仓库公开的 GitHub Releases。正式版默认仅检查正式版，预发布版默认包含预发布；复选框会更改本次会话的渠道并立即检查。版本按语义版本号比较；由于 main 构建复用正式版的基础版本号，同一基础版本的正式版与 main 构建之间在发布日期均可用时按发布日期判断新旧。仅接受包含已上传完成的 macOS arm64 DMG 的发布。启动时不自动联网检查，不发送媒体、文件路径或模型凭据。

发现新版后，点击“打开 GitHub 下载页”，在浏览器下载对应 DMG。等待导出完成，退出应用，再替换 Applications 中的旧版。设置、模型配置和安全存储中的凭据保持原位置；编辑会话不会在重启后恢复。这是手动下载安装更新，不会自动下载、退出或安装。安装包仍为 ad-hoc 签名、未公证，macOS 可能需要在“隐私与安全性”中选择“仍要打开”。
...
水印默认位于左下角，字号为 24 个输出像素。水印仅作用于当前选中的片段：开关、文字、字体、描边、位置、字号和边距独立保存。切换片段会恢复各自设置，拼接导出也仅在对应片段中绘制水印。复制片段会复制其设置，之后可分别修改。
